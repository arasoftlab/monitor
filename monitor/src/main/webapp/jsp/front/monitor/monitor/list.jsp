...
function getTestPopupPage(id){
  ...
  <h3>모니터링과제(현장과제)</h3>
		...
		<table class="pc_notice_table" style="width:100%;background:#fff;border:none;padding:0;border-spacing:0;">
			<tr>
				<td>
				<!-- 카테고리 -->
							...
							<c:when test="${!empty list }">
								<c:forEach var="item" items="${list }">
									<tr align='center' style="cursor:pointer" >
										...
										<td class='bbsno' align='left'>
										
											<c:choose>
												<c:when test="${empty member}">
													<span class="monitor_subject" style="text-align:left">${item.title }</span>
												</c:when>
												<c:otherwise>
													<c:choose>
														<c:when test="${item.status eq '02' }">
															<span class="monitor_subject" style="text-align:left"><a href='javascript:alert("공지사항에서 상세내용을 확인해 주세요.");'>${item.title }</a></span>
														</c:when>
														<c:when test="${item.status eq '04' }">
															<span class="monitor_subject" style="text-align:left"><a href='<c:url value='/front/monitoring/monitoring/view.do?subject_id=${item.subject_id }'/>'  style="color: blue;font-weight: bold;">${item.title }</a></span>
														</c:when>
														<c:otherwise>
															<span class="monitor_subject" style="text-align:left"><a href='<c:url value='/front/monitoring/monitoring/view.do?subject_id=${item.subject_id }'/>'>${item.title }</a></span>
														</c:otherwise>
													</c:choose>														
												</c:otherwise>
											</c:choose>
										...
										<td class="bbsetc_dateof_write">
											<fmt:formatDate value='${item.apply_s_date }' pattern='yy/MM/dd' />~<fmt:formatDate value='${item.apply_e_date }' pattern='yy/MM/dd' />
										</td>
										...
										<td class="bbsetc_dateof_write">
											<c:choose>
												<c:when test="${item.status eq '01' }">준비중</c:when>
										...
										<td class="bbsetc_dateof_write">
											<c:choose>
													<c:when test="${item.send_cnt eq item.question_cnt }">
														<b>완료</b>
													</c:when>
													
													<c:when test="${item.send_cnt > 0 }">
														${item.send_cnt}
													</c:when>
