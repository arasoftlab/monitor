...
function sso_sysnc()
{
	...
	if(id != "null")
	{
                $.ajax({
                        async : true,
                        type : "POST",
                        url : "<c:url value='/front/info/join/getbackendmember.do'/>",
                        data : $("#sso_id_form").serialize(),
...
function member_update(form){
	...
	$.ajax({
		async : true,
		type : "POST",
		url : "<c:url value='member_update.do'/>",
		data : $(form).serialize(),
		success : function(data) {
			if (data.result == "success") {
				alert('수정되었습니다.');
...
function sojaeji( sido , gugun, dong) {
	...
	<h3>▥ 기수정보</h3>
		<table style="width:100%;background:#fff;border:none;border-spacing:0;padding:0;">
			<tr>
				<td>
					<table class="table_02 mypage_table" style="text-align:center;margin-bottom:60px;">
						<colgroup>
							<col width="30%;">
							<col width="20%;">
							<col width="50%;">
						...
						<tbody>
							...
							<tr>
								<td>제 ${a_vo.poll_num } 기</td>

								<td>
									<c:if test="${a_vo.poll_num eq vo.poll_num }">

										<c:if test="${vo.grade eq 'nomal' }">
											선정
										</c:if>
								...
								<td>
									<fmt:formatDate value="${a_vo.poll_start_date }" pattern="yyyy.MM.dd" /> ~ 					
									<fmt:formatDate value="${a_vo.poll_end_date }" pattern="yyyy.MM.dd" />
								</td>
							</tr>
		...
		<div class="col-lg-12" style="padding:0;">
			<h3 style="float:left;">▥ 서울시 회원정보</h3>
		</div>
		<div class="col-lg-12" style="float:left;padding:0;">
			<label class="information"><span style="color:#4374D9;">※ 서울시 통합회원 정보에 등록(수정)하신 내용으로 조회됩니다.</font></span></label>
		</div>
					...
					<table class="table_02 mypage_table">
						...
						<tbody>
							<tr>
								<th>* 이름(ID)</th>
								<td>
								<c:out value="${fn:substring(info.name, 0, 1)}" />**
								<span style="margin-left:26%;">(아이디) <span style="margin-left:1%;">
								<c:out value="${fn:substring(info.id, 0, 3)}" />*****
								</span></td>
							</tr>
							<tr>
								<th>* 연락처</th>
								<td>
										...
										<c:out value="${fn:substring(info.phone, 0, 1)}" />** -

										<c:out value="${fn:substring(info.phone, 4, 5)}" />*** -
									
										<c:out value="${fn:substring(info.phone, 9, 10)}" />*** 
									
									</span>
									<div class="radioGroup">
										<input type="radio" disabled id="phone_type" value="01"	<c:if test="${info.phone_type eq '01' }">checked</c:if>/> 집&nbsp;&nbsp; 
										<input type="radio" disabled id="phone_type" value="02"	<c:if test="${info.phone_type eq '02' }">checked</c:if>/> 직장&nbsp;&nbsp; 
										<input type="radio" disabled id="phone_type" value="03"	<c:if test="${info.phone_type eq '03' }">checked</c:if>/> 휴대폰&nbsp;&nbsp; 
									</div>
								</td>
							</tr>
							<tr>
								<th>* 이메일</th>
								<td>
									<c:out value="${fn:substring(info.email, 0, 3)}" />****@********
								</td>
							</tr>
							<tr>
								<th>* 거주지</th>
							...
							<tr>
								<th>* 생년월일</th>
								<td>
									<c:out value="${fn:substring(info.birthday, 0, 2)}" />******
								</td>
							</tr>
						</tbody>
				...
				<td>
					...
					<table class="table_02 mypage_table">
						...
						<tbody>
							<tr>
								<th> * 아이디</th>
								<td><c:out value="${fn:substring(info.id, 0, 3)}" />*****</td>
							</tr>
							<tr>
								<th>* 이름</th>
								<td><c:out value="${fn:substring(info.name, 0, 1)}" />**</td>
							</tr>
							<tr>
								<th>* 연락처</th>
								<td>
							...
							<tr>
								<th>* 이메일</th>
								<td><c:out value="${fn:substring(info.email, 0, 3)}" />****@********</td>
							</tr>
							<tr>
								<th>* 거주지</th>
								<td>${info.sido}</td>
							...
							<tr>
								<th>* 생년월일</th>
								<td><c:out value="${fn:substring(info.birthday, 0, 2)}" />******</td>
							</tr>
						</tbody>
		...
		<div style="text-align:right;margin-top:20px;">
			<button type="button" class="btn btn-primary button_blue button_white" onclick="location.href='https://www.seoul.go.kr/seoul/servlet/seoul?div=mem&a=UserInfoViewRenewalApp'">개인정보수정</button>
		</div>
		
		<div style="margin-top:20px; padding:0;">
			<h3>▥ 추가입력정보</h3>
			<label class="information"><span style="color:#4374D9;">※ 과제별 모니터요원 선정 시 참고자료 및 통계처리에 활용</span></label>
      <label class="information"><span style="color:#4374D9;">※ 성별정보는 노출하지 않으며 수정이 불가하기에 잘못 선택하신 경우에는 담당자에게 연락 바랍니다.</span></label>							
		</div>
		...
		<table class="" style="width:100%;background:#fff;border:none;border-spacing:0;padding:0;">
			<tr>
				<td>
					...
					<table class="table_02 mypage_table">
						...
						<tbody>
									<select style="height:100%;width:40%;display:none" name="sex">
										<option value="M" <c:if test="${vo.sex eq 'M'}">selected</c:if>>남자</option>
										<option value="W" <c:if test="${vo.sex eq 'W'}">selected</c:if>>여자</option>
									</select>
						...
						<!--  
							<tr>
								<th>* 성 별</th>
								<td>
									<select style="height:100%;width:40%;" name="sex">
										<option value="남자" <c:if test="${vo.sex eq '남자'}">selected</c:if>>남자</option>
										<option value="여자" <c:if test="${vo.sex eq '여자'}">selected</c:if>>여자</option>
									</select>
								</td>
							</tr>
						-->
							<tr>
								<th>* 직업</th>
								<td>
									<select style="height: 25px;width:40%;" name="job">
										<option value="">(선택)</option>
										<option value="가정주부" <c:if test="${vo.job eq '가정주부'}">selected</c:if>>가정주부</option>
										<option value="대학생" <c:if test="${vo.job eq '대학생'}">selected</c:if>>대학생</option>
										<option value="회사원" <c:if test="${vo.job eq '회사원'}">selected</c:if>>회사원</option>
										<option value="전문,자유직" <c:if test="${vo.job eq '전문,자유직'}">selected</c:if>>전문,자유직</option>										
										<option value="자영업" <c:if test="${vo.job eq '자영업'}">selected</c:if>>자영업</option>
										<option value="무직" <c:if test="${vo.job eq '무직'}">selected</c:if>>무직</option>
										<option value="교육직" <c:if test="${vo.job eq '교육직'}">selected</c:if>>교육직</option>
							...
							<tr>
								<th>* 상세거주지</th>
								<td>
									<select name="sido" id="sido" style="height: 20px;display:none;"><option value="">-시도-</option></select>
									...
									<c:choose>
										...
										<c:when test="${!empty vo}">																		
											<script>
												sojaeji('${vo.sido}', '${vo.gugun}' , '${vo.dong}');									
											</script>
										</c:when>
			...
			<tr>
				<td>
					<!-- 게시판 끝 -->
					<div style="text-align:right;margin-top:20px;">
						<button type="button" class="btn btn-info button_blue button_white" onclick="member_update('#w_form')">추가정보수정</button>
						<c:set value="list.do" var="path" />
						<button type="button" class="btn btn-default button_blue button_white" onclick="location.href='${path}'">취소</button>
					</div>
