						...
						<c:forEach var="item" items="${optionList }" varStatus="i">
							<div id="${item.options_id }" class="optionList row "
								...
								<div class="col-xs-2"  style="padding-left: 10px;">
									<div class="col-xs-6">
										<a id="btn_del" class="btn btn-default icon_btn delete" onclick="fnDeleteOption('${item.options_id}');">
				                        ...
				                        <div >
												...
												<c:choose>
													<c:when test="${!empty item.fileList }">
														<input type="file" name="O_file_setI${item.options_id}" onchange="O_fnFileCheck_update('I${item.options_id}' , '${item.cont_uuid}' , '_${vo.question_id}')" />
													</c:when>
													<c:otherwise>
														<input type="file" name="O_file_setI${item.options_id}" onchange="O_fnFileCheck('I${item.options_id}' , '${item.cont_uuid}' , '_${vo.question_id}')" />
													</c:otherwise>
												</c:choose>
...
-->								
								...
								<div class="col-xs-12">
										<div id="O_fileListI${item.options_id }">
											<c:forEach var="file" items="${item.fileList }">
												<img src="<c:url value='${file.savePath }/${file.unqFileName }'/>" class='img-responsive img-thumbnail' style="max-width: 95%;">
											</c:forEach>
										</div>
									</div>							    
					...
					<div id="answerAdd" class="pull-left col-xs-12" >
