																		...
																		<i class="fa fa-1x fa-picture-o"> 																	
																			<c:choose>
																				<c:when test="${!empty item.fileList }">
																					<input type="file" accept="image/*" capture="camera" class="file_input" name="QS_file_setV${item.idx}" onchange="QS_fnFileCheck_update('V${item.idx}' , '${item.cont_uuid}' , '_${vo.question_id}')"/>
																				</c:when>
																				<c:otherwise>
																					<input type="file" accept="image/*" capture="camera" class="file_input" name="QS_file_setV${item.idx}" onchange="QS_fnFileCheck('V${item.idx}' , '${item.cont_uuid}' , '_${vo.question_id}')"/>
																				</c:otherwise>																
																			</c:choose>																	
																		</i>
																	</div>
