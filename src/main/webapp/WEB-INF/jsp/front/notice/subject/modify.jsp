...
<script>
	...
	function onDelte()
	{
		$.ajax({
			async : true,
			type : "POST",
			url : "<c:url value='/front/notice/subject/delete.do'/>",
			data : $("#web").serialize(),
	...
	function onTempChk_W(location)
	{
		...
		if (location == "U") {
			$.ajax({
				async : true,
				type : "POST",
				url : "<c:url value='/front/notice/subject/update.do'/>",
				data : $("#web").serialize(),
				success : function(data){
			...
			$.ajax({
				async : true,
				type : "POST",
				url : "<c:url value='/front/notice/subject/apply.do'/>",
				data : $("#web").serialize(),
				success : function(data){
			...
			<tr>
				...
				<!-- 카테고리 -->
					<table class="table_02 modify">
						...
						<tr>
							<td colspan="2">
								<textarea style="width:100%;" rows="3" name="description">${m_vo.description}</textarea>
							</td>
						</tr>
					</table>
			...
			<tr>
				...
				<!-- 카테고리 -->
					<table class="table_02 modify">
						...
						<tr>
							<th colspan="2">${vo.subjectVO.title }</th>
						</tr>
						<tr>
							<td colspan="2">
							<div style="margin-left:5px; margin-right:5px; margin-top:10px; margin-bottom:10px;">
						...
						<tr>
							<td colspan="2">
								<textarea style="width:100%;" rows="10" name="description">${m_vo.description}</textarea>
							</td>
						</tr>
					</table>
						...
						<c:choose>
							<c:when test="${!empty list }">
    							...
    							<c:if test="${member_id eq item.member_id }">
											<tr>
												<td><c:out value="${fn:substring(item.member_id, 0, 3)}" />*****</td>
												<td><c:out value="${fn:substring(item.member_name, 0, 1)}" />**</td>
												<td>${item.poll_num}</td>
